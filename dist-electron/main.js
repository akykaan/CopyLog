import { ipcMain, app, globalShortcut, screen, clipboard, BrowserWindow } from "electron";
...
const copyBoardSlice = createSlice({
  name: "board",
  ...
  reducers: {
    ...
    togglePin: (state, action) => {
      const itemId = state.items.findIndex(
        (item2) => item2.id === action.payload
      );
      const item = state.items[itemId];
      state.items[itemId] = item.pinned ? { ...item, pinned: false } : { ...item, pinned: true };
  ...
  selectors: {
    ...
    selectFilteredItems: (state) => {
      ...
      return state.items.filter(
        (item) => item.text.toLowerCase().includes(state.searchText.toLowerCase())
      );
    },
    selectSearchText: (state) => state.searchText
  }
});
const clipBoardReducer = copyBoardSlice.reducer;
const initialState = {
  isCopy: {}
};
const copyItemSlice = createSlice({
  name: "copy",
  initialState,
  reducers: {
    ...
    resetCopyStatus: (state, action) => {
      const { [action.payload.id]: _, ...rest } = state.isCopy;
      state.isCopy = { ...rest };
    }
  }
});
const copyReducer = copyItemSlice.reducer;
